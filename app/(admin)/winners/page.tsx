  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 px-4 pb-4 sm:px-6 lg:px-8">
        <div className="flex min-h-0 flex-col rounded-xl border border-ink-200 bg-white shadow-card">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ink-100 px-5 py-4">
            <div className="flex items-center gap-2.5">
              <h2 className="text-base font-semibold text-ink-900">Revision de envios</h2>
              <Badge tone="warning">
                {formatNumber(winners.filter((c) => (c.shippingStatus ?? "pending") === "pending").length)} por revisar
              </Badge>
            </div>
          </div>
          <ClaimsReview claims={winners} />
        </div>
